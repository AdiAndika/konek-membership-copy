...
const isOtpComplete = computed(() =>
  otp.value.every((val) => val.match(/^\d$/))
);
...
const verifyOtp = async () => {
  if (!isOtpComplete.value || isLoading.value || !phoneNumberState.value)
    return;
  ...
  try {
    ...
    errorMessage.value =
      error.message || "Kode OTP tidak sesuai atau sudah kedaluwarsa.";
  } finally {
...
<template>
  <div
    v-if="isSuccess"
    class="fixed inset-0 bg-white flex flex-col justify-center items-center"
  >
    <Transition
      appear
    ...
    >
      <div class="w-full max-w-sm flex flex-col items-center text-center">
        ...
        />
        <h1 class="text-2xl font-bold text-gray-800">Anda Berhasil Log In</h1>
      </div>
    </Transition>
  </div>
        ...
        <div class="w-full max-w-sm">
          <div class="flex justify-center mb-6">
            <div class="bg-blue-100 p-4 rounded-full">
              <img
                src="~/assets/images/otp-wa-logo.svg"
                alt="WhatsApp Icon"
                class="w-10 h-10"
              />
            </div>
