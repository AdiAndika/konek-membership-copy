...
<template>
  <section class="mb-12">
    ...
    <div v-else class="divide-y divide-gray-200 bg-white p-4 rounded-xl shadow-sm">
      <div v-for="(item, index) in products" :key="index" class="flex justify-between items-center py-3">
        <img :src="item.image" :alt="item.title" class="w-24 h-24 object-contain" />
        
        <button v-if="status === 'active'" @click="emit('open-modal', item)" class="text-[#0080FF] font-semibold hover:underline">
          Lihat akun
        </button>
